...
@app.get("/items/")
async def read_item(skip: int = 0, limit: int = 10):  # skip 和 limit 是查询参数
...
async def read_item(item_id: str, q: str | None = None):  # q 是可选的查询参数
    if q:
...
当你为非路径参数 (目前, 我们只看到了查询参数) 声明默认值时, 它就不是必需的。
...
@app.get("/items/{item_id}")
async def read_user_item(item_id: str, needy: str):  # needy 是必需的查询参数
...
async def read_user_item(
    item_id: str,
    needy: str,            # 必需的 str
    skip: int = 0,         # 默认为 0 的 int
    limit: int | None = None  # 可选的 int
):
